    ...
    public function handle()
    {
        ...
        foreach ($events as $event) {
            ...
            if (!$event->is_notified) {
                ...
                if ($timeSetting === '24h' && $startsAt->copy()->subHours(24)->lte($now)) $shouldNotify = true;
                elseif ($timeSetting === '12h' && $startsAt->copy()->subHours(12)->lte($now)) $shouldNotify = true;
                elseif ($timeSetting === '2h' && $startsAt->copy()->subHours(2)->lte($now)) $shouldNotify = true;
                elseif ($timeSetting === '0m' && $startsAt->lte($now)) $shouldNotify = true;
            ...
            if ($event->is_notified && !$event->is_repeat_notified && $user->notify_repeat !== 'none') {
                ...
                if ($repeatSetting === '1h' && $startsAt->copy()->subHours(1)->lte($now)) $shouldRepeat = true;
                elseif ($repeatSetting === '30m' && $startsAt->copy()->subMinutes(30)->lte($now)) $shouldRepeat = true;
                elseif ($repeatSetting === '15m' && $startsAt->copy()->subMinutes(15)->lte($now)) $shouldRepeat = true;
