...
@section('content')
    <div class="max-w-4xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
        ...
        <div
            class="bg-white rounded-[2rem] border border-zinc-100 shadow-[0_4px_20px_-4px_rgba(0,0,0,0.02)] p-6 sm:p-8 mb-12 relative z-10">
            ...
            <div class="grid grid-cols-7 gap-px bg-zinc-100 border border-zinc-100 rounded-2xl overflow-hidden">
                ...
                @foreach ($days as $day)
                    ...
                    <div
                        class="bg-white p-2 min-h-[5rem] sm:min-h-[6rem] flex flex-col items-center gap-1.5 transition-colors hover:bg-zinc-50/80 cursor-default group">

                        <span
                            class="w-7 h-7 flex items-center justify-center font-mono text-sm transition-all duration-200
            ...
            <div class="flex flex-col gap-3">
                ...
                @empty
                    <div
                        ...
                        <p class="text-sm font-medium text-zinc-900 mb-1">Задач нет</p>
                        <p class="text-sm text-zinc-500 mb-5 text-center max-w-[35ch]">В этом месяце пока нет активных
                            событий. Нажмите на + в нижнем меню, чтобы добавить задачу.</p>
                    </div>
                @endforelse
            </div>
        </div>
        ...
        @if ($pastEvents->count() > 0)
            <div>
                <h2 class="text-xl font-medium tracking-tight text-zinc-400 mb-6 flex items-center gap-3">
                    Прошедшие события
                </h2>
                ...
                <div class="flex flex-col gap-3">
                    @foreach ($pastEvents as $event)
                        <div
                            class="group relative flex items-center justify-between p-4 rounded-2xl bg-white/50 border border-zinc-100 transition-all duration-300 opacity-60 grayscale hover:grayscale-0 hover:opacity-100 hover:bg-white hover:shadow-[0_2px_10px_-4px_rgba(0,0,0,0.02)]">
                            ...
                            <div class="flex items-center gap-4">
                                ...
                                <div class="flex flex-col">
                                    <span
                                        class="text-base font-medium text-zinc-600 decoration-zinc-300">{{ $event->title }}</span>
                                    <span class="text-sm font-mono text-zinc-400">
                                        {{ \Carbon\Carbon::parse($event->starts_at)->locale('ru')->translatedFormat('d F H:i') }}
                                    </span>
                                </div>
